...
export function ImprovedAIQuizDraft({ quizTitle, onQuestionsGenerated, onClose }: ImprovedAIQuizDraftProps) {
    ...
    try {
      ...
      const response = await fetch('/api/ai/generate-quiz', {
        ...
        body: JSON.stringify({
          ...
          goals: formData.context || undefined
        })
      })
      ...
      if (questions.length === 0) {
        throw new Error('Inga frågor genererades. Försök justera dina parametrar.')
      }
      
      setGeneratedQuestions(questions)
      setSelectedQuestions(new Set(questions.map((q: Question) => q.id)))
      setStep('preview')
      
    } catch (error) {
      console.error('AI generation error:', error)
      const errorMessage = error instanceof Error ? error.message : 'Okänt fel uppstod'
      setErrorMessage(`Kunde inte generera frågor: ${errorMessage}`)
      setStep('error')
    }
  }
  ...
  const handleSaveAsDraft = async () => {
    if (selectedQuestions.size === 0) {
      setErrorMessage('Välj minst en fråga att spara som utkast.')
      return
    }
    ...
    try {
      ...
      const difficultyPrefix = difficultyMap[formData.difficulty] || 'Fördjupat'
      
      const draftTitle = `${difficultyPrefix} ${formData.subject} - ${formData.gradeLevel}`
      const draftDescription = `AI-genererat quiz med ${questionsToSave.length} frågor om ${formData.subject}${formData.topics ? `, med fokus på: ${formData.topics}` : ''}`

      const response = await fetch('/api/quiz/save-draft', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Okänt fel' }))
        throw new Error(errorData.error || 'Kunde inte spara utkast')
      }
      ...
      alert(`Quiz "${draftTitle}" har sparats som utkast!`)
      onClose()
      
    } catch (error) {
      console.error('Save draft error:', error)
      setErrorMessage(`Kunde inte spara utkast: ${error instanceof Error ? error.message : 'Okänt fel'}`)
    } finally {
      setIsSaving(false)
    }
  }
            ...
            <div className="flex gap-3">
              ...
              {step === 'preview' && (
                <>
                  ...
                  >
                    {isSaving ? 'Sparar...' : `Spara ${selectedQuestions.size} som utkast`}
                  </Button>
                  <Button
